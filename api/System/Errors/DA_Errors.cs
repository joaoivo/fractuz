...
public static class DA_Errors{
	public static IEnumerable<EN_Error> Select(IConfiguration config, out int? totalRowCount, out int? seachRowCount, out int? searchPageCount, out string? query, ref int? pageNumber, ref int? pageRowCount, String? columnsOrderBy=null
		...
		IEnumerable<EN_Error> error_lst = new List<EN_Error>();
		...
		using (SqlConnection db = new SqlConnection(config["Database:Default"])){
			error_lst = db.Query<EN_Error>("[dbo].[pr_SystemErrors_sel]",parameters);
		}
	...
	public static EN_Return Insert(IConfiguration config,EN_Error error){
		EN_Return error_return = new EN_Return();
		DynamicParameters parameters = new DynamicParameters();
		...
		parameters.Add("@pAppStackTrace"		, error.AppStackTrace	, DbType.String	, ParameterDirection.Input,8000);
		...
		parameters.Add("@pExtraData"			, error.ExtraData			, DbType.String	, ParameterDirection.Input,8000);
		...
		using (SqlConnection db = new SqlConnection(config["Database:Default"])){
			...
			error_return.id = parameters.Get<Guid?>("@rGuid");
			error_return.description = parameters.Get<string>("@rProcessMessage");

			error_return.isSuccess = parameters.Get<Boolean>("@rIsOK");
			error_return.isError = !error_return.isSuccess;

			error_return.tittle = (parameters.Get<Boolean>("@rIsOK")?"Inserção efetuada com sucesso":"Erro na tentativa de inserção");
		}
		return error_return;
	}
	...
	public static EN_Return Delete(IConfiguration config,Guid SystemIDX){
		...
		using (SqlConnection db = new SqlConnection(config["Database:Default"])){
			...
			error_return.tittle = (parameters.Get<Boolean>("@rIsOK")?"Exclusão efetuada com sucesso":"Erro na tentativa de Exclusão");
		}
		return error_return;
	}
}
