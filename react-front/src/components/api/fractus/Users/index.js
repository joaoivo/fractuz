...
export const useApiFractuzUsers= ()=>{
	...
	const getTokenRenovation = async (token) => {
		const jsonResponse = await methodGet({Authorization:"Bearer "+token },systemConfig.urls.PUBLIC_API_URL + apiFractuzEndPoint.loginRenovation);
		if(jsonResponse.isError){throw new ExceptionSystemApiFractuz(`O servidor Acusou erro: Message: ${jsonResponse.description}`);}
		return jsonResponse;
	};

	return {getLoginToken, getTokenRenovation};
}